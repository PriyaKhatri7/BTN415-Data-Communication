...
Server_UDP::Server_UDP(std::string ip, int port)
{
    start_dlls();
    num_nodes++;
    this->active_socket = INVALID_SOCKET;
...
bool Server_UDP::bind_socket()
{
    ...
    SvrAddr.sin_addr.s_addr = inet_addr(this->ip.c_str());
    SvrAddr.sin_port = htons(this->port);
    if (bind(this->active_socket, (struct sockaddr*)&SvrAddr, //bind server socket active to address ip and port from constructor
        sizeof(SvrAddr)) == SOCKET_ERROR) {
        std::cout << "Could not bind socket to address" << std::endl;
        return false;
    }
...
{
    return send(this->active_socket, message.c_str(), message.length(), 0); //sent to client with matching ip and port number
}

sockaddr_in Server_UDP::recieve_message(std::string& message)
{
    char RxBuffer[MAX_BUFFER_SIZE] = {};
    memset(RxBuffer, 0, MAX_BUFFER_SIZE);
    int num_bytes = recv(this->active_socket, RxBuffer, MAX_BUFFER_SIZE, 0);
...
sockaddr_in Client_UDP::recieve_message(std::string& message)
{
     char RxBuffer[MAX_BUFFER_SIZE] = {};
    memset(RxBuffer, 0, MAX_BUFFER_SIZE);
    int num_bytes = recv(this->active_socket, RxBuffer, MAX_BUFFER_SIZE, 0);
