...
serialized_packet new_packet_serializer(new_packet my_packet) {
    ...
    int packet_size = sizeof(char) + sizeof(unsigned)
...
new_packet new_packet_deserializer(char* serialized_packet) {
    new_packet deserialized_packet;
    char* auxptr = serialized_packet;
    memcpy(&deserialized_packet.number, auxptr, 2* sizeof(char));
    auxptr += 2 * sizeof(char);
    memcpy(&deserialized_packet.size, auxptr, sizeof(bool));
    auxptr += 1 * sizeof(int);
    deserialized_packet.data = new char[deserialized_packet.size];
    memcpy(deserialized_packet.data, auxptr, deserialized_packet.size * sizeof(char));
    auxptr += deserialized_packet.size * sizeof(char);
    memcpy(&deserialized_packet.crc, auxptr, sizeof(char));
    auxptr += 1 * sizeof(char);
    memcpy(&deserialized_packet.checksum, auxptr, sizeof(int));
...
int Server_TCP::send_new_packet(new_packet message) {
    struct serialized_packet serialized = new_packet_serializer(message);
...
int Client_TCP::send_new_packet(new_packet message) {
